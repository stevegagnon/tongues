...
export function reverb(param, sample_rate) {
  ...
  return (buffer) => {
		...
		for delay in (param.delaymin+1..param.delaymax+1).rev() {
			let random = random_data[param.seed + delay];
			// Is there an echo with this delay?
			if (random as u64 * (delay - param.delaymin) as u64) >> 32 < (param.nbufs - b) as u64 {
				...
				for i in 0..size {
					...
					outputs[c][i] += out as f32 * param.volumes[c];
			...
			feedback *= param.decay_mul as f64;
		}
